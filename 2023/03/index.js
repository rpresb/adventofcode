...
function part2() {
  ...
  while (tmpData.indexOf("*") > -1) {
    tmpData = tmpData.replace("*", "!");

    const lines = tmpData.trim().split("\n");
    const engineParts = lines.reduce((prev, line, lineIndex) => {
      const numbers = line.match(/\d+/g);
      let lastEnd = 0;

      const valid = numbers?.filter((num) => {
        const start = line.indexOf(num, lastEnd);
        const end = start + num.length;
        lastEnd = end;

        let isAdjacent = getIsAdjacent(start, line, end, lineIndex, lines, "!");
    ...
    if (engineParts.length === 2) {
      result += engineParts.reduce((a, b) => a * b, 1);
    }
...
function part1() {
  ...
  return lines.reduce((prev, line, lineIndex) => {
    const numbers = line.match(/\d+/g);
    let lastEnd = 0;
    const valid = numbers?.filter((num) => {
      const start = line.indexOf(num, lastEnd);
    ...
    return prev + +(valid?.reduce((a, b) => +a + +b, 0) || 0);
  }, 0);
}
...
function getIsAdjacent(start, line, end, lineIndex, lines, symbol) {
  let isAdjacent = false;

  if (start > 0) {
    const prevChar = line.substring(start - 1, start).replace(/\./g, "");
    isAdjacent = symbol ? prevChar === symbol : prevChar.length > 0;
    // console.log("prevChar", line, start, end, num, prevChar);
  }

  if (!isAdjacent && end < line.length - 1) {
    const nextChar = line
      .substring(end, Math.min(end + 1, line.length))
      .replace(/\./g, "");
    isAdjacent = symbol ? nextChar === symbol : nextChar.length > 0;
    // console.log("nextChar", line, start, end, num, nextChar);
  }

  if (!isAdjacent && lineIndex > 0) {
    const prevLine = lines[lineIndex - 1];
    const prevLineString = prevLine
      .substring(Math.max(0, start - 1), Math.min(end + 1, prevLine.length))
      .replace(/\./g, "");

    isAdjacent = symbol ? prevLineString === symbol : prevLineString.length > 0;
  }

  if (!isAdjacent && lineIndex < lines.length - 1) {
    const nextLine = lines[lineIndex + 1];
    const nextLineString = nextLine
      .substring(Math.max(0, start - 1), Math.min(end + 1, nextLine.length))
      .replace(/\./g, "");

    isAdjacent = symbol ? nextLineString === symbol : nextLineString.length > 0;
  }
  return isAdjacent;
}
