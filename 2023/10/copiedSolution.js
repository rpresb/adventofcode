module.exports.part2 = function (mapMatrix) {
  ...
  const HARDCODED_PIPE_UNDER_S_SIGN = "|";
  const HARDCODED_SIGN_DIRECTIONS = [
    [0, -1],
    [0, 1],
  ];
  ...
  const getMapEntries = (mapMatrix) =>
    Object.entries(mapMatrix).flatMap(([row, x]) =>
      Object.entries(x).map(([col, ch]) => [parseInt(row), parseInt(col), ch])
    );
  const [row, col, _] = getMapEntries(mapMatrix).find(
    ([_, __, ch]) => ch === "S"
  );
  ...
  while (queue.length > 0) {
    ...
    if (!visited.has(getKey(row, col))) {
      ...
      queue.push([
        row,
        col,
        getStepByPipe[mapMatrix[row][col]]([x, y]),
        distance + 1,
      ]);
    }
  }
  ...
  const zoomMapX2 = (map) => {
    const zoomedMap = Array.from({ length: map.length * 2 }, () =>
      Array.from({ length: map[0].length * 2 }, () => ".")
    );

    getMapEntries(map).forEach(([row, col, v]) => {
      zoomedMap[row * 2][col * 2] = v;
      zoomedMap[row * 2][col * 2 + 1] = interpolateHorizontal(
        v,
        map[row]?.[col + 1]
      );
      zoomedMap[row * 2 + 1][col * 2] = interpolateVertical(
        v,
        map[row + 1]?.[col]
      );
    });
  ...
  function interpolateHorizontal(a, b) {
    if (a == "-" && b == "-") return "-";
    if (a == "F" && b == "7") return "-";
    if (a == "F" && b == "J") return "-";
    if (a == "F" && b == "-") return "-";
  ...
  function interpolateVertical(a, b) {
    if (a == "|" && b == "|") return "|";
    if (a == "F" && b == "J") return "|";
    if (a == "F" && b == "L") return "|";
    if (a == "F" && b == "|") return "|";
  ...
  const onlyLoopMapSize2x = zoomMapX2(
    mapMatrix.map((x, row) =>
      x.map((y, col) =>
        visited.has(getKey(row, col)) ? mapMatrix[row][col] : "."
      )
    )
  );
  const loopNodes2x = new Set(
    getMapEntries(onlyLoopMapSize2x)
      .filter(([_, __, ch]) => ch !== ".")
      .map(([row, col, _]) => getKey(row, col))
  );
  ...
  while (toVisitOuter.length > 0) {
    ...
    [
      ...
      .filter(
        ([xNext, yNext]) =>
          mapSize2x[yNext]?.[xNext] !== undefined &&
  ...
  return getMapEntries(mapSize2x).filter(
    ([row, col, _]) =>
      !outerVisited.has(`${row}:${col}`) &&
