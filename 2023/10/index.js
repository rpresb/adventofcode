...
module.exports = () => {
  ...
  console.log("Part 2 ===>", copiedSolution.part2(matrix));
};
...
function getNextFromS(matrix, position) {
  // check right
  const right = matrix[position[0]][position[1] + 1];
  if (["-", "J", "7"].includes(right)) {
    return [[position[0], position[1] + 1], position];
  }

  // check left
  const left = matrix[position[0]][position[1] - 1];
  if (["-", "F", "L"].includes(left)) {
    return [[position[0], position[1] - 1], position];
  }

  // check top
  const top = matrix[position[0] - 1][position[1]];
  if (["|", "F", "7"].includes(top)) {
    return [[position[0] - 1, position[1]], position];
  }
}

function isValidConnection(matrix, position, direction) {
  // '|',  '-',  'L',  'J',  '7',  'F',  '.',  'S',
...
function getNextPos(matrix, position, prevPos) {
  const direction = [position[0] - prevPos[0], position[1] - prevPos[1]];

  let nextPos = [];

  const topPos = [position[0] - 1, position[1]];
  const topVal = (matrix[topPos[0]] || [])[topPos[1]];
  const rightPos = [position[0], position[1] + 1];
  const rightVal = matrix[rightPos[0]][rightPos[1]];
  const bottomPos = [position[0] + 1, position[1]];
  const bottomVal = (matrix[bottomPos[0]] || [])[bottomPos[1]];
  const leftPos = [position[0], position[1] - 1];
  const leftVal = matrix[leftPos[0]][leftPos[1]];

  const [validCurrent, place] = isValidConnection(matrix, position, direction);

  // left valid
  if (place === "left" && validCurrent.includes(leftVal)) {
    nextPos = leftPos;
  }

  // top valid
  else if (place === "top" && validCurrent.includes(topVal)) {
    nextPos = topPos;
  }

  // right valid
  else if (place === "right" && validCurrent.includes(rightVal)) {
    nextPos = rightPos;
  }

  // bottom valid
  else if (place === "bottom" && validCurrent.includes(bottomVal)) {
    nextPos = bottomPos;
  }
...
function part1(matrix) {
  ...
  do {
    if (matrix[currentPos[0]][currentPos[1]] === "S") {
      if (countLoop === 0) {
        [currentPos, prevPos] = getNextFromS(matrix, currentPos);
        continue;
      } else {
        // back to S
        console.log("back to S");
        break;
      }
    }

    [currentPos, prevPos] = getNextPos(matrix, currentPos, prevPos);

    countLoop++;
  } while (true);
  return Math.ceil(countLoop / 2);
}

function part2(matrix) {
  const start = getStart(matrix);

  const matrixCopy = matrix.map((line) => line.map((item) => item));
  ...
  do {
    if (matrix[currentPos[0]][currentPos[1]] === "S") {
      if (countLoop === 0) {
        ...
        // back to S
        console.log("back to S");
        matrixCopy[currentPos[0]][currentPos[1]] = "X";
        break;
      }
    }
  ...
  return countInnerLoop(matrixCopy);
}

function countInnerLoop(matrix) {
  const isSurroundedByX = (i, j, spaceIsBad) => {
    let foundX = false;
    // top
    for (let tmp = i - 1; tmp > 0; tmp--) {
      if (["X", "I"].includes(matrix[tmp][j])) {
        foundX = true;
        break;
      }

      if (spaceIsBad && matrix[tmp][j] === " ") {
        return false;
      }
    }
    ...
    // right
    foundX = false;
    for (let tmp = j + 1; tmp < matrix[i].length; tmp++) {
      if (["X", "I"].includes(matrix[i][tmp])) {
        foundX = true;
        break;
      }

      if (spaceIsBad && matrix[i][tmp] === " ") {
        return false;
      }
    }
    ...
    // bottom
    foundX = false;
    for (let tmp = i + 1; tmp < matrix.length; tmp++) {
      if (["X", "I"].includes(matrix[tmp][j])) {
        foundX = true;
        break;
      }
  ...
  // clear I
  const clearOutsideI = (matrix) => {
    for (let i = 0; i < matrix.length; i++) {
      for (let j = 0; j < matrix[i].length; j++) {
        if (matrix[i][j] === "I" && !isSurroundedByX(i, j, true)) {
          matrix[i][j] = " ";
        }
      }
    }
  ...
  for (let i = 0; i < 100; i++) {
    clearOutsideI(matrix);
  }

  let count = matrix.reduce((total, line) => {
    const numOfI = line.filter((letter) => letter === "I").length;

    return total + numOfI;
  }, 0);

  console.log(count, matrix.map((line) => line.join("")).join("\n"));
