module.exports = function () {
  ...
  const games = data
    ...
    .reduce((prev, curr) => {
      ...
      const plays = parts[1]
        ...
        .split(";")
        .map((play) => {
          return play.split(",").reduce((prev, colors) => {
            const colorsParts = colors.trim().split(" ");
            return {
              ...prev,
              [colorsParts[1]]: +colorsParts[0],
            };
          }, {});
        });

      return { ...prev, [parts[0]]: plays };
    }, {});

  const sum = Object.keys(games)
    .map((gameKey) => {
      const game = games[gameKey];
      const id = +gameKey.replace("Game ", "");

      const isPossible = game.every((g) => {
        return Object.keys(input).every((key) => {
          const res = input[key] >= (g[key] ?? 0);
          return res;
        });
      });
  ...
  const fewer = Object.values(games).reduce((prev, game) => {
    const gameMax = game.reduce((maxVal, colors) => {
      const maxColors = { ...maxVal };
      Object.keys(colors).forEach((color) => {
        maxColors[color] = Math.max(maxVal[color] ?? 0, colors[color]);
      });
      return maxColors;
    }, {});

    const power = Object.values(gameMax).reduce((a, b) => {
      return a * b;
    }, 1);
