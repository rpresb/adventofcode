...
function exec() {
  console.log(
    input
      ...
      .split(`\n`)
      .map((line) => {
        const { first, last } = /(?<first>\d)(.*(?<last>\d))?/gm.exec(
          line
        ).groups;
  ...
  console.log(
    input
      ...
      .map((line) => {
        ...
        for (let i = 0; i < lineReplaced.length - 2; i++) {
          for (let j = i + 2; j <= Math.min(lineReplaced.length, i + 5); j++) {
            const part = lineReplaced.substring(i, j);
            if (Object.keys(mapNumbers).includes(part)) {
              lineReplaced = lineReplaced.replace(part, mapNumbers[part]);
            }
          }
        }

        const { first } = /(?<first>\d)/gm.exec(lineReplaced).groups;

        lineReplaced = line;
        for (let i = lineReplaced.length - 3; i >= 0; i--) {
          for (let j = i + 2; j <= Math.min(lineReplaced.length, i + 5); j++) {
            const part = lineReplaced.substring(i, j);
            if (Object.keys(mapNumbers).includes(part)) {
              lineReplaced = `${lineReplaced.slice(0, i)}${
                mapNumbers[part]
              }${lineReplaced.slice(j)}`;
            }
          }
        }

        const { last } = /.*(?<last>\d)/gm.exec(lineReplaced).groups;
