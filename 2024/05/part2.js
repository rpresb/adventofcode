...
export default function exec() {
  ...
  const sumMiddle = updates.reduce((acc, update) => {
    ...
    let isValid = isValidRule(arr, rulesMap);

    if (!isValid) {
      // call function to swap invalid elements
      isValidRule(arr, rulesMap, true);
...
function isValidRule(arr, rulesMap, swapIfInvalid = false) {
  let isValid = true;
