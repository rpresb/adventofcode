  ...
  for (let antenna of Object.keys(antennas)) {
    ...
    for (let i = 0; i < points.length - 1; i++) {
      for (let j = i + 1; j < points.length; j++) {
        ...
        const doubleBefore = [y1 - dy, x2 > x1 ? x1 - dx : x1 + dx];
        const doubleAfter = [y2 + dy, x2 > x1 ? x2 + dx : x2 - dx];

        if (matrix[doubleBefore[0]]?.[doubleBefore[1]]) {
          antinodes.add(doubleBefore.join(","));
        }

        if (matrix[doubleAfter[0]]?.[doubleAfter[1]]) {
